...
load("births/births_columbia.Rda")
...
pop_col <- pop_col[hom_rate, on = .(region, year)]
pop_col[, crude_adjusted_exposure := ifelse(sex == "males", exposure * 1.1, exposure)]
pop_col[, war_adjusted_exposure := ifelse(sex == "males", exposure * weight, exposure)]
...
births[, asfr_male_ca := birth_father / crude_adjusted_exposure_males]
births[, asfr_male_wa := birth_father / war_adjusted_exposure_males]
...
tfrs_col <- births[, .(mac_female = sum(asfr_female * middle_age) / sum(asfr_female),
           ...
           tfr_male_ca = sum(asfr_male_ca * 5),
           tfr_male_wa = sum(asfr_male_wa * 5),
...
tfrs_col[, tfr_ratio_wa := tfr_male_wa / tfr_female]
tfrs_col[, tfr_ratio_ca := tfr_male_ca / tfr_female]
tfrs_col[, mac_diff := mac_male - mac_female]

# Save the result
save(tfrs_col, file = "U:/projects/3 Regional birth squeezes/Subnational_birth_squeezes/tfr_col.Rda")
save(asfr_col, file = "U:/projects/3 Regional birth squeezes/Subnational_birth_squeezes/asfr_col.Rda")
write.csv(tfrs_col, "tfr_regional.csv")


### National TFRS ------------------------------------------

# Estimate the national tfrs
asfr_nat <- births[, .(asfr_male = sum(birth_father) / sum(exposure_males), 
           asfr_female = sum(birth_mother) / sum(exposure_females),
           asfr_male_ca = sum(birth_father) / sum(crude_adjusted_exposure_males),
           asfr_male_wa = sum(birth_father) / sum(war_adjusted_exposure_males)), by = .(year, age_group)]

# Estimate the tfrs
tfr_nat <- asfr_nat[, .(tfr_male = sum(asfr_male * 5),
                      tfr_male_ca = sum(asfr_male_ca * 5),
                      tfr_male_wa = sum(asfr_male_wa * 5),
                      tfr_female = sum(asfr_female * 5)), 
                  by = .(year)]
...
# Estimate the tfr ratio and the male to female age difference
tfr_nat[, tfr_ratio := tfr_male / tfr_female]
tfr_nat[, tfr_ratio_wa := tfr_male_wa / tfr_female]
tfr_nat[, tfr_ratio_ca := tfr_male_ca / tfr_female]

# Save the result
write.csv(tfr_nat, "tfr_national.csv")
...
ggsave(last_plot(), filename = "U:/projects/3 Regional birth squeezes/Subnational_birth_squeezes/figures/map_col_tfr_ratio.pdf")
### END ############################################